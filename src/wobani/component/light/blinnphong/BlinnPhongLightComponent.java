...
/**
 This abstract class stores the light's diffuse, specular and ambient components.
 */
public abstract class BlinnPhongLightComponent extends Component{
    ...
    /**
     Sets the diffuse color to the given value. All of diffuse color's components must be min. 0.
    ...
    /**
     Sets the specular color to the given value. All of specular color's components must be min. 0.
    ...
    /**
     Sets the ambient color to the given value. All of ambient color's components must be min. 0.
    ...
    /**
     Signs that the at least one of the light's properties changed and have to update it in the shader.
     */
    @Internal
    ...
    /**
     Creates a FloatBuffer which contains all the light's parameters like diffuse color, position etc. The FloatBuffer's
     content must follow the memory layout of the shader's light struct.
    ...
    /**
     Creates an IntBuffer which contains all the light's meta data like the light's type etc. The IntBuffer's content
     must follow the memory layout of the shader's light struct.
    ...
    public String toString(){
        StringBuilder res = new StringBuilder().append(super.toString()).append("\n")
                .append(BlinnPhongLightComponent.class.getSimpleName()).append("(").append(" diffuse color: ")
                .append(diffuseColor).append(", specular color: ").append(specularColor).append(", ambient color: ")
                .append(ambientColor).append(")");
        return res.toString();
    }
