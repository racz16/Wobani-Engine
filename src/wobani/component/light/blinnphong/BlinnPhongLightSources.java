...
/**
 Stores the Blinn-Phong light sources and refreshes them in the VRAM if necessary.
 */
public class BlinnPhongLightSources{
    ...
    /**
     If more than half of a SSBO is empty or even the whole SSBO is empty since a time more than this time limit, the
    ...
    /**
     Refreshes in the VRAM all the Blinn-Phong light sources changes since the last frame. If the VGA side objects are
     released, this method recreates them.
     */
    ...
    /**
     Tries to optimize the tiles' SSBOs. If more than half of a SSBO is empty or even the whole SSBO is empty since a
    ...
    /**
     Refreshes the given directional light source in the VRAM. It adds, removes or updates the light if necessary.
    ...
    /**
     Adds the given parameter to the list of directional lights (if it isn't contained yet).
    ...
    /**
     Removes the given light if it's not the main directional light and not already removed.
    ...
    /**
     Adds the given light if it's the main directional light and not already added.
    ...
    /**
     Refreshes the given positional light source in the VRAM. It adds, removes or updates the light if necessary.
    ...
    /**
     Adds the given parameter to the list of positional lights (if it isn't contained yet).
    ...
    /**
     Adds, removes or updates the light source in the previous tile if necessary.
    ...
    /**
     Adds, removes or updates the light source in the current tile if necessary. If the current tile doesn't exists, it
     creates the tile.
    ...
    /**
     Returns the given light's current tile. If it doesn't exist, this method creates a new tile and adds the light to
     it.
    ...
    /**
     Returns the given light source's current tile. If the light isn't attached to a GameObject, this method returns null
     value.
    ...
    /**
     Binds the given tile's SSBO to the binding point. If the tile is null, this method binds the zero SSBO to the
     binding point.
    ...
    /**
     Returns the 4 closest tile centers based on the given closest tile center and offset.
    ...
    /**
     Returns the offset for the closest tiles. You can get the 4 closest tiles by adding to the closest tile (0; 0),
     (offset.x; 0), (0; offset.y), (offset.x; offset.y).
    ...
    /**
     ...
     @param absolutePositionCoordinate  one of the absolute position's coordinates
    ...
    private static int computeOffsetCoordinate(int closestTileCenterCoordinate, float absolutePositionCoordinate){
        if(java.lang.Math.abs(closestTileCenterCoordinate + tileSize - absolutePositionCoordinate) < java.lang.Math
                .abs(closestTileCenterCoordinate - tileSize - absolutePositionCoordinate)){
            return tileSize;
        }else{
            return -tileSize;
        }
    ...
    /**
     Computes one of the tile's (center) position coordinates based the given absolute position coordinate.
    ...
    /**
     Creates a new UBO for the main directional light if the last UBO isn't usable.
     */
    ...
    /**
     Creates a new zero SSBO if the last one isn't usable and fills it with a 0.
     */
