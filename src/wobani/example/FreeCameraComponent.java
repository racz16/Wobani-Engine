...
/**
 This class extends the DefaultCameraComponent and allows the user to control the camera movement and rotation by the
 keyboard.
...
public class FreeCameraComponent extends CameraComponent{
    ...
    /**
     Sprint boost. Note that the final speed is move speed multiplied by this sprint speed. So sprint boost is not the
     final speed, it's just a multiplying factor.
     */
    private float sprintBoost = 1.5f;

    /**
     This method allows the user to control camera movement and rotation the following ways: W - move forward, A - move
     left, S - move backward, D - move right, Q - turn left, E - turn right, SHIFT - boost movement speed.
     */
    @Override
    public void update(){
        if(getGameObject() != null){
            Vector3f forwardSpeed = getGameObject().getTransform().getForwardVector()
                    .mul(moveSpeed * Time.getDeltaTimeFactor());
            Vector3f rightSpeed = getGameObject().getTransform().getRightVector()
                    .mul(moveSpeed * Time.getDeltaTimeFactor());
    ...
    /**
     Returns sprint boost. Note that the final speed is move speed multiplied by this sprint speed. So sprint boost is
     not the final speed, it's just a multiplying factor.
    ...
    /**
     Sets the sprint boost to the given value. Note that the final speed is move speed multiplied by this sprint speed.
     So sprint boost is not the final speed, it's just a multiplying factor.
    ...
    public String toString(){
        StringBuilder res = new StringBuilder().append(super.toString()).append("\n").append("FreeCameraComponent(")
                .append(" move speed: ").append(moveSpeed).append(", rotation speed: ").append(rotateSpeed).append(")");
        return res.toString();
    }
