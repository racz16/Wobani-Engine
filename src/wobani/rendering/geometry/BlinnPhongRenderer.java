...
/**
 This GeometryRenderer can draw meshes and splines by using the Blinn-Phong shading. You can fill the materials with
 diffuse color or diffuse map, specular color or specular map and normal map. If you set the appropirate parameters, the
 specular map's alpha channel used as the glossiness value and the normal map's alpha channel as a parallax map. If you
 don't fill the diffuse or specular slots, the shader uses default values (basically you can even use this
 GeometryRenderer with an empty material).
...
public class BlinnPhongRenderer extends GeometryRenderer{
    ...
    public void render(){
        ...
        for(Renderable renderable : renderables.getRenderables(renderer)){
            ...
            for(int i = 0; i < renderables.getRenderableComponentCount(renderer, renderable); i++){
                ...
                if(renderableComponent.isActive() && renderableComponent.isRenderableActive() && Utility
                        .isInsideMainCameraFrustumAabb(renderableComponent)){
    ...
    private void beforeDrawInstance(@NotNull RenderableComponent rc){
        ...
        shader.loadObjectUniforms(transform.getModelMatrix(), new Matrix3f(transform.getInverseModelMatrix()), rc
                .isReceiveShadows());
