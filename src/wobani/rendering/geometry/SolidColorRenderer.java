...
/**
 It can render RenderableContainer by drawing them with one color. Shadows and lighting not affect the final color. It
 only uses the material's diffuse color, if there is no diffuse color in the material, it uses as default the mid-gray
 color.
 */
public class SolidColorRenderer extends GeometryRenderer{
        ...
        for(Renderable renderable : renderables.getRenderables(renderer)){
            ...
            for(int i = 0; i < renderables.getRenderableComponentCount(renderer, renderable); i++){
                ...
                if(renderableComponent.isActive() && renderableComponent.isRenderableActive() && Utility
                    ...
                    beforeDrawRenderableInstance(renderableComponent.getMaterial(), renderableComponent.getGameObject()
    ...
    private void beforeDrawRenderableInstance(@NotNull Material material, @NotNull Matrix4f modelMatrix){
        ...
        Vector4f color = slot != null && slot.isActive() && slot.getColor() != null ? slot
                .getColor() : new Vector4f(0.5f);
