...
public class EnvironmentMapRenderer extends PrepareRenderer{
    ...
    public void render(){
        ...
        for(int probeIndex = 0; probeIndex < Scene.getComponentLists()
                .getComponentCount(DynamicEnvironmentProbeComponent.class); probeIndex++){
            DynamicEnvironmentProbe probe = Scene.getComponentLists()
                    .getComponent(DynamicEnvironmentProbeComponent.class, probeIndex).getProbe();
                    ...
                    for(Renderable renderable : renderables.getRenderables(renderer)){
                        ...
                        for(int i = 0; i < renderables.getRenderableComponentCount(renderer, renderable); i++){
                            ...
                            if(renderableComponent.isActive() && renderableComponent
                                    .isRenderableActive() && renderableComponent.getBoundingShape()
                                    .getRealRadius() >= probe.getMinSize() && renderableComponent.getGameObject()
                                    .getTransform().getAbsolutePosition().distance(probe.getPosition()) <= probe
                                    .getMaxDistance()){
