...
public class ShadowRenderer extends PrepareRenderer{
    ...
    /**
     The directional light's shadow camera's distance from the user's camera's center.
     */
    ...
    /**
     Sets the shadow camera's distance from the user's camera's center to the given value.

     @param shadowCameraDistance the shadow camera's distance from the user's camera's center

     @throws IllegalArgumentException the parameter have to be higher than zero
     */
    ...
    /**
     ...
     @throws IllegalArgumentException near plane's distance must be higher than 0 and it can't be higher than the far
     plane's distance
     */
    ...
    /**
     ...
     @throws IllegalArgumentException far plane's distance must be higher than the near plane's distance
     */
    ...
    private void refresh(){
        if(isActive()){
            ...
            projectionViewMatrix.set(Utility.computeShadowMapProjectionViewMatrix(Scene.getParameters()
                    .getValue(BlinnPhongRenderer.MAIN_DIRECTIONAL_LIGHT)
                    .getGameObject(), getShadowCameraDistance(), getShadowCameraNearDistance(), getShadowCameraFarDistance()));
            ...
            for(Renderable renderable : renderables.getRenderables(renderer)){
                ...
                for(int i = 0; i < renderables.getRenderableComponentCount(renderer, renderable); i++){
                    ...
                    if(renderableComponent.isActive() && renderableComponent.isRenderableActive() && renderableComponent
                            .isCastShadow() && isInsideFrustum(renderableComponent)){
                        beforeDrawMeshInstance(projectionViewMatrix, renderableComponent.getGameObject().getTransform()
                                .getModelMatrix());
    ...
    /**
     Unbinds the FBO and prepares the pipeline to the next stage of the rendering.
     */
    ...
    /**
     Determines whether the given mesh component is inside the directional light's view frustum.

     @param renderableComponent mesh component

     @return true if the mesh component is inside the directional light's view frustum, false otherwise
     */
    private boolean isInsideFrustum(@NotNull RenderableComponent renderableComponent){
        ...
        return isInsideFrustum(renderableComponent.getBoundingShape().getRealAabbMin(), renderableComponent
    ...
    /**
     Removes the shader program and the FBO from the GPU's memory. After this method call you can't use this
     GeometryRenderer.
     */
    @Override
    public void release(){
    ...
    public String toString(){
        return super.toString() + "\nShadowRenderer{" + "shader=" + shader + ", fbo=" + fbo + '}';
    }
