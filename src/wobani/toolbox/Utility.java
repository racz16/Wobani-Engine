...
public class Utility{
    ...
    /**
     Returns the engine's root logger. You get the same result if call Logger.getLogger("wobani").
    ...
    /**
     Sets the logging level for the engine's root logger and all of it's handlers to the given value.
     <p>
    ...
    /**
     ...
     @throws IllegalArgumentException fov must be in the (1;179) interval, the near plane must be higher than 0 and the
    ...
    public static Matrix4f computePerspectiveProjectionMatrix(float fov, float nearPlane, float farPlane){
        ...
        return new Matrix4f().setPerspective(toRadians(fov), Window.getAspectRatio(), nearPlane, farPlane);
    }
    ...
    /**
     ...
     @throws IllegalArgumentException scale must be higher than 0, the near plane must be higher than 0 and the far plane
     higher than the near plane
     */
    @NotNull
    ...
    /**
     Computes the main directional light's projection view matrix for shadow mapping.
     ...
     @throws IllegalArgumentException all parameters must be positive and nearDistance must be lower than farDistance
     */
    @NotNull
    ...
    /**
     Returns whether the specified Collections contains reference to the given object.

     @param collection collection
     @param object     object

     @return true if the given collection contains reference to the given object, false otherwise
     */
    ...
    /**
     Removes the given object from the given Collection if the Collection stored reference to the object.

     @param <T>        type
     @param collection collection
     @param object     object

     @return true if the given object successfully removed from the Collection, false otherwise
     */
    ...
    /**
     Converts an angle measured in degrees to an approximately equivalent angle measured in radians. The conversion from
    ...
    /**
     Converts an angle measured in radians to an approximately equivalent angle measured in degrees. The conversion from
    ...
    /**
     Determines whether the given RenderableComponent is inside the main camera's view frustum (computing by AABB).

     @param renderableComponent RenderableComponent

     @return true if the RenderableComponent is inside the main camera's view frustum, false otherwise
     */
    public static boolean isInsideMainCameraFrustumAabb(@NotNull RenderableComponent<?> renderableComponent){
        ...
        if(camera != null){
            return camera.isInsideFrustum(renderableComponent.getBoundingShape().getRealAabbMin(), renderableComponent
                    .getBoundingShape().getRealAabbMax());
        }else{
            return false;
        }
    }
    ...
    /**
     Determines whether the given RenderableComponent is inside the main camera's view frustum (computing by bounding
     sphere).
    ...
    public static boolean isInsideMainCameraFrustumSphere(@NotNull RenderableComponent<?> renderableComponent){
        Camera camera = Scene.getParameters().getValue(Scene.MAIN_CAMERA);
        if(camera != null){
            return camera.isInsideFrustum(renderableComponent.getGameObject().getTransform()
                    .getAbsolutePosition(), renderableComponent.getBoundingShape().getRealRadius());
        }else{
            return false;
        }
    }
    ...
    /**
     ...
     @return true if the given Resource isn't null and is usable, false otherwise
     */
    ...
    /**
     This class can compute the main directional light's projection view matrix for shadow mapping. It tries to compute
     the view matrix to best fit to the main camera's view frustum.
     */
    private static class ShadowMapMatrixSolver{
        ...
        /**
         Maximum x coordinate of the main camera's frustum corner points in light space.
         */
        private static float lightSpaceXMax;
        /**
         Minimum x coordinate of the main camera's frustum corner points in light space.
         */
        private static float lightSpaceXMin;
        /**
         Maximum y coordinate of the main camera's frustum corner points in light space.
         */
        private static float lightSpaceYMax;
        /**
         Minimum y coordinate of the main camera's frustum corner points in light space.
         */
        ...
        /**
         Computes the main directional light's projection view matrix based on the given data.
        ...
        private static void initializeLight(@NotNull GameObject lightGameObject, float distance){
            ...
            lightPosition = camera.getFrustumCenter()
                    .add(lightGameObject.getTransform().getForwardVector().negate().mul(distance));
        }
        ...
        /**
         Refreshes the light space minimum and maximum values based on the camera's frustum corner points.
         */
        ...
        /**
         Refreshes the light space minimum values based on the camera's given frustum corner point.
        ...
        /**
         Refreshes the light space maximum values based on the camera's given frustum corner point.
        ...
        /**
         Refreshes the light's position based on the light space minimum and maximum values.
         */
        ...
        /**
         Computes the main directional light's projection view matrix based on the already computed light space min, max
         and position values.
        ...
        private static Matrix4f computeMatrix(float near, float far){
            ...
            Matrix4f lightProjectionMatrix = new Matrix4f()
                    .setOrtho(-horizontalScale, horizontalScale, -verticalScale, verticalScale, near, far);
