 ...
 * a dynamic resource (not loaded from file), like an FBO, a generated texture,
 * etc., it gets a globally unique id which identifys the resource. The paths
 * list will be empty and the index will be 0.<br>
 * If you load one resource from one or more files, like a like a simple texture
 * or a cubemap texture, the id and the index will be 0, and the file paths will
 * identify the resource.<br>
 * If you load one file to more resources (like a model to meshes), the id will
 * be 0, the path will be the file's path and the index will be the resource's
 * index in the file (like the mesh's index in the model). In that case the
 * file's path and the index will identify the resource.
 */
public class ResourceId {
    ...
    /**
     * Initializes a new ResourceId to the given values. Note that it sets the
     * index to 0, if the number of paths is greater than 1. Only dynamic
     * resources (not loaded from a file) get unique id.
     *
    ...
    /**
     * Returns the resource's id. It can be 0 if the resource is not dynamic
     * (loaded from files) In that case file paths identify the resource. If
     * it's dynamic, this id is globally unique.
     *
    ...
    /**
     * Returns the index of the resource. If the resource is dynamic (not loaded
     * from a file) or the file loaded to just one resource it returns 0. If a
     * file loaded to various resources it returns this resource's index in the
     * file.
     *
    ...
    /**
     * Creates the specified number of ResourceIds. It can be useful when you
     * load 1 file and than segment it to numerous resources like when you load
